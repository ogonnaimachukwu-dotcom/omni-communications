/**
 * Outlook / Exchange Inbox Adapter — IMAP over OAuth2 (Microsoft Identity)
 *
 * Uses imapflow with XOAUTH2 mechanism against Outlook IMAP servers.
 * Health check uses Microsoft Graph API for richer validation.
 */
...
interface OutlookOAuthCredentials {
  email: string;
  accessToken: string;
}
...
export class OutlookInboxAdapter implements InboxAdapter {
  readonly name = "oauth_outlook";

  constructor(private readonly creds: OutlookOAuthCredentials) {}
  ...
  async disconnect(): Promise<void> {
    // Stateless — fresh client per operation
  }
  ...
  async health(): Promise<{ status: "healthy" | "unhealthy"; details?: string }> {
    try {
      ...
      return { status: "unhealthy", details: `Microsoft Graph: ${res.status} ${body.slice(0, 200)}` };
    } catch (err) {
      return {
        status: "unhealthy",
        details: err instanceof Error ? err.message : "Unknown Outlook error",
      };
    }
  }
      ...
      try {
        ...
        for await (const msg of client.fetch(uidsToFetch, {
          ...
          messages.push({
            ...
            threadId: null, // Outlook conversation ID not in IMAP; use References chain
